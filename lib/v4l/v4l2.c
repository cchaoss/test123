...
/**
 * The main capturing thread  主捕捉线
 * This thread handles the queue and dequeue of buffers, to make sure only the latest
 * image buffer is preserved for image processing.这个线程处理缓冲区的入队和出队，确保最后的图像缓冲区被保存以便后续的图像处理。
 * @param[in] *data The Video 4 Linux 2 device pointer
 * @return 0 on succes, -1 if it isn able to fetch(取) an image,
...
struct v4l2_device *v4l2_init(char *device_name, uint16_t width, uint16_t height, uint8_t buffers_cnt) {
  ...
  fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_UYVY;  //old  fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_UYVY;
