...
/**
 * Create a new image，即配置结构体image_t的一个实体
...
/**
 * Copy an image from inut to output，拷贝图像，源图像与目标图像格式须一致
...
void image_copy(struct image_t *input, struct image_t *output)
{
  ...
  memcpy(&output->ts, &input->ts, sizeof(struct timeval));   //复制结构体，memcpy遇字符结束标志停止复制
  memcpy(output->buf, input->buf, input->buf_size);          //复制图像缓冲区
}

/**
 * This will switch image *a and *b，交换两幅图像
...
/**
 * Convert an image to grayscale(灰度).将YUV422图像转换为灰度图像。
...
/**
 * Filter colors in an YUV422 image，对一张YIV422图像过滤掉颜色
...
/**
* Simplified high-speed low CPU downsample function without averaging，简化高速CPU下采样功能不平均
...
/**
 * This outputs a subpixel window image in grayscale。输出子像素的灰度图像
 * Currently only works with Grayscale images as input but could be upgraded to
 * also support YUV422 images.目前仅能用于灰度图像，但也可以升级来支持YUV422图像。
...
/**
 * Calculate the  gradients using the following matrix:用下面的矩阵计算梯度（图像源为灰度图像）
 * [0 -1 0; -1 0 1; 0 1 0]
...
/**
 * Calculate the G vector of an image gradient，计算一个图像梯度的G向量
...
/**
 * Calculate the difference between two images and return the error，计算两幅图像的不同并返回错误，只用于灰度图像。
...
/**
 * Calculate the multiplication（积） between two images and return the error
...
/**
 * Show points in an image by coloring them through giving 着色显示图像中的点
...
/**
 * Shows the flow from a specific point to a new point，显示一个特定点到某一个点的流
...
/**
 * Draw a line on the image，在一副图像上画线
