...
struct opticflow_t {
  ...
  float prev_phi;                   ///< Phi from the previous image frame(图像帧)
  float prev_theta;                 ///< Theta from the previous image frame
  struct image_t img_gray;          ///< Current gray image frame(灰度图像帧)
  struct image_t prev_img_gray;     ///< Previous gray image frame
  struct timeval prev_timestamp;    ///< Timestamp(时间戳) of the previous frame, used for FPS calculation
  ...
  uint8_t subpixel_factor;          ///< The amount of subpixels per pixel(每个像素子像素的数量)
  uint8_t max_iterations;           ///< The maximum amount of iterations(迭代) the Lucas Kanade algorithm should do
  uint8_t threshold_vec;            ///< The threshold(阈值) in x, y subpixels which the algorithm should stop

  bool_t fast9_adaptive;            ///< Whether the FAST9 threshold should be adaptive(自适应)
